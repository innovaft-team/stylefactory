...
/**
 * Strips the article bodies before a post list is serialised into the page.
 *
 * The list pages render only the cover image and the title, but the full
 * `cro.content` / `eng.content` markdown for every post was being shipped in
 * `__NEXT_DATA__` in both languages — around 21 KB of HTML on /trends alone.
 * Titles are kept because `getPostTitle` and the slug helpers need them.
 */
export function stripPostContent<T extends Blog>(blogs: T[]): T[] {
    return blogs.map((blog) => ({
        ...blog,
        cro: { ...blog.cro, content: "" },
        eng: { ...blog.eng, content: "" },
    }));
}
